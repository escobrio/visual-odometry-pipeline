...
class VOVisualizer:
    ...
    def __init__(self, 
        ...
        self.ax_3d.view_init(elev=-30., azim=-90)
        ...
        self.current_pose_2d_scatter = self.ax_2d.scatter([], [], c='r', s=50, label='Current Pose')
    ...
    def update_3d_view(self, landmarks: np.ndarray, poses: List[np.ndarray]):
        ...
        if len(poses) > 0:
            ...
            self.traj_line.set_data_3d(poses_xyz[:, 0], poses_xyz[:, 1], poses_xyz[:, 2])
            ...
            self.ax_3d.set_xlim(cx - self.CAMERA_VIEW_RANGE, cx + self.CAMERA_VIEW_RANGE)
            self.ax_3d.set_ylim(cy - self.CAMERA_VIEW_RANGE, cy + self.CAMERA_VIEW_RANGE)
            self.ax_3d.set_zlim(cz - self.CAMERA_VIEW_RANGE, cz + self.CAMERA_VIEW_RANGE)
    ...
    def _update_2d_view(self, landmarks: np.ndarray, poses: List[np.ndarray]):
        ...
        if len(poses) > 0:
            ...
            current_pose = poses[-1]
            self.current_pose_2d_scatter.set_offsets([[current_pose[0, 3], current_pose[2, 3]]])
