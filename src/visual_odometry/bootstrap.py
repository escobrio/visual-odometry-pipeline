...
def bootstrap_VO(images_paths, cfg, camera_intrinsics, visualizer):
    ...
    if cfg.cfg["bin"]["use_binning"]:
        ...
        quota_per_bin = _allocate_quota(
            cfg.cfg["bin"]["total_initial_keypoints_count"], bin_weights
        )
        ...
        points_0 = cv2.goodFeaturesToTrack(
            prev_image, mask=None, **cfg.feature_params()
        )
    ...
    while median_depth < 0.0 or median_depth > 7.0:
        ...
        points_i, st, err = cv2.calcOpticalFlowPyrLK(
            prev_image, next_image, prev_points, None, **cfg.lk_params()
        )  # Nx1x2 points
        ...
        retval, R_iW, i_t_iW, inlier_mask = cv2.recoverPose(
            E, points_0, points_i, camera_intrinsics, mask=mask_essential
        )  # Change of basis frame_0 to frame_i | expresses frame_0 in frame_i
        ...
        if cfg.visualize and visualizer is not None:
            ...
            # visualizer.refresh()
            visualizer.step(
                next_image, points_i, points_0, frame_idx, landmarks_3d.T, [np.eye(4)]
            )
