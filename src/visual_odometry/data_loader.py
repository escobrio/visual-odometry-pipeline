...
def load_dataset(ds):
    ...
    malaga_path = os.path.join(
        project_root, "data", "provided_data", "malaga-urban-dataset-extract-07"
    )
    parking_path = os.path.join(project_root, "data", "provided_data", "parking")
    own_dataset_path = os.path.join(
        project_root, "data", "own_rec_dataset", "frames_vga_house"
    )
    ...
    if ds == 0:
        ...
        last_frame = len(left_images) if len(left_images) > 0 else 598
        ...
        K = np.array([[fx_vga, 0, cx_vga], [0, fy_vga, cy_vga], [0, 0, 1]])
    else:
