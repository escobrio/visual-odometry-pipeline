...
class VisualOdometryPipeline:
    def __init__(self, cfg: VOConfig):
        self.cfg = cfg
    ...
    def visual_odometry(self):
        ...
        images_paths, last_frame, K = load_dataset(self.cfg.dataset_id)
        ...
        Rot, Translation, landmarks_i, keypoints_i, frame_idx = bootstrap_VO(
            images_paths, self.cfg, K, visualizer
        )
        ...
        if self.cfg.cfg["initialization_part2"]["auto_swap_xy_if_y_gt_x"]:
        ...
        if not self.cfg.cfg["bin"]["use_binning"]:
            fraction = self.cfg.cfg["initialization_part2"][
                "fraction_of_features_as_candidates"
            ]
        ...
        n_frames = min(self.cfg.n_frames, last_frame)
        ...
        for frame_idx in range(frame_idx + 1, n_frames):
            ...
            if self.cfg.cfg["pipeline"]["log"]:
                ...
                reproj_errors = np.linalg.norm(
                    P_next_candidates - projected_points, axis=1
                )
            ...
            S, new_landmarks, info_new_landmarks = add_new_landmarks(
                S, image, image_next, K, global_camera_poses, self.cfg.cfg
            )
